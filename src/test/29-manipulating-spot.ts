...
describe("manipulating spot", () => {
  ...
  describe("spot_region status of a region", function() {
    ...
    it(
      "should not change the spot_region to false if a fabric's spot primary is that region"
    );
  });

  describe("spot fabric", () => {
    let fabricName = `Fabric${Date.now()}`;
    after(() => {
    ...
    it("should create a fabric with a spot primary region", async () => {
      ...
      const response = await fabric.createFabric(
        fabricName,
        undefined,
        options
      );
  ...
  describe("spot collection", async () => {
    ...
    describe("mapulating spot collection's region", () => {
      ...
      it(
        "should change the spot region of a spot collection to some other region"
      );
    });
  });
});
