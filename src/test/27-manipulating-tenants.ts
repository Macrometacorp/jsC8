...
describe('Manipulating tenants', function () {
    ...
    describe('tenant.create', () => {
        ...
        before(async () => {
            const tenantName: string = `testTenant_${Date.now()}`;
        ...
        it("tenant gets replicated", (done) => {
            ...
            Promise.all(promises).then(tenants => {
                for (let i = 0; i < tenants.length; ++i) {
                    const elem = tenants[i].result.find((ten: TenantListObj) => ten.tenant === tenant.name);
    ...
    describe('tenant.dropTenant', () => {
        ...
        beforeEach(async () => {
            const password = "1234";
            const tenantName = `testTenant_${Date.now()}`;
            tenant = fabric.tenant(tenantName);
