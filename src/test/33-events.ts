...
describe("Manipulating events", function () {
    ...
    describe("event.details", () => {
        ...
        it("throws error when eventId is not set", async () => {
            const event = fabric.event();
            assert.throws(event.details.bind(event), Error, "Event id is not set. Either provide while creating the handler or create a new event.");
        });
    });
});
