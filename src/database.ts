...
import { c8SearchView, C8View, constructView, ViewType } from "./view";
...
export class Database {
  ...
  exists(): Promise<boolean> {
    return this.get().then(
      () => true,
      err => {
        if (isC8Error(err) && err.errorNum === DATABASE_NOT_FOUND) {
          return false;
        }
        throw err;
      }
  ...
  c8SearchView(viewName: string): c8SearchView {
    return new c8SearchView(this._connection, viewName);
  }
